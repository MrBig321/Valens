...
class PlaceDB:
	...
	def read(self):
		...
		try:
			...
			try:
				while(True):
					lines.append(pickle.load(f, fix_imports=True))	#python2.x compatible because of Morinus (fix_imports)
				f.close()
	...
	def write(self):
		try:
			...
			for rec in self.placedb:
				...
				pickle.dump(txt, f, fix_imports=True, protocol=2)	#python2.x compatible because of Morinus (fix_imports)

			f.close()

		except IOError:
			messagebox.showerror(message=texts.txtsfiles['DBFileError'])
