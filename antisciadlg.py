...
class AntisciaDlg:
	...
	def __init__(self, parent, chrt, opts):
		...
		self.win = Toplevel()
		self.win.title(texts.txtsantiscia['Antiscia'])
...
#		tree.column('#0', width=100, minwidth=2000, anchor='center') #!! Horizontal scrollbar only takes minwidth into account !!
	...
	#	tree.heading('#0', text='Planets')
		tree.heading('long', text=texts.txtsantiscia['Longitude'])
		...
		num = len(self.chart.antis.ascmclons)
		for i in range(num):
			lon = self.chart.antis.ascmclons[i]
		...
		num = len(self.chart.antis.plslons)
		for i in range(num):
			lon = self.chart.antis.plslons[i]
			if (self.options.ayanamsa != 0):
				lon -= self.chart.ayanamsa
				lon = util.normalize(lon)
			d, m, s = util.decToDeg(lon)
		...
		num = len(self.chart.antis.lotslons)
		for i in range(num):
			lon = self.chart.antis.lotslons[i]
			if (self.options.ayanamsa != 0):
				lon -= self.chart.ayanamsa
				lon = util.normalize(lon)
			d, m, s = util.decToDeg(lon)
